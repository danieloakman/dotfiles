# Deprecated: use my.programs.gws (Google Workspace CLI) instead.
{ env, config, pkgs, lib, ... }:
let
...
in
{
  options.my.programs.gcalcli.enable = lib.mkEnableOption ''
    Enable the Google Calendar CLI tool (gcalcli).

    Deprecated: use my.programs.gws instead.
  '';

  config = lib.mkIf cfg.enable {
    warnings = [
      ''
        my.programs.gcalcli is deprecated and will be removed.
        Use my.programs.gws instead (Google Workspace CLI includes calendar via `gws calendar`).
      ''
    ];
