...
export class XYDoTool extends Singleton {
  ...
  protected ydotoolDaemon = once(async () => {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const handler = (data: Buffer) => {
        ...
        if (/you're advised to run this program as root|failed to open uinput/i.test(msg)) reject(msg);
        else if (!init) {
          init = true;
          resolve();
  ...
  protected init = once(async () => {
    if (isOnWayland && XYDoTool.initYdoToolDaemon) await this.ydotoolDaemon();
  });
  ...
  private cmd(x: number | string, y: number | string) {
    if (isOnWayland) return `${XYDoTool.initYdoToolDaemon ? 'sudo ' : ''}ydotool mousemove -x ${x} -y ${y}`;
